...
const MobileNav = () => {
  ...
  return (
    <div className="sm:hidden">
      ...
      >
        <button type="button" aria-label="toggle modal" className="fixed w-full h-full cursor-auto focus:outline-none" onClick={onToggleNav}></button>
        <nav className="fixed h-full mt-8">
          {HEADER_NAV_LINK.map((link) => (
            <div key={link.title} className="px-12 py-4">
              <div onClick={onToggleNav}>
                <Link href={link.href}>
                  <a className="text-2xl font-bold tracking-widest text-gray-900 dark:text-gray-100">{link.title}</a>
                </Link>
              </div>
            </div>
          ))}
